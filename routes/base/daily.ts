...
module.exports = {
  ...
  async ['/textbook_list']({ req, res, request, cheerio }) {
    const { id } = req.query
    const html = await request.send(`http://www.kekenet.com/menu/13216/${id == 6 ? '' : `List_${id}.shtml`}`)
    const $ = cheerio.load(html)
    const regx = /http:\/\/www.kekenet.com\/menu\/(.*?)\.shtml/
    const result = $('#menu-list li').toArray().map((item: any) => {
      const id = $(item).find('h2').find('a').eq(1).attr('href').match(regx)[1]
      const title = $(item).find('h2').find('a').eq(1).text().trim().replace('(MP3+中英字幕)', '').split(':')
      const date = $(item).find('p').text().trim().match(/(.{4})-(.{2})-(.{2})/)[0]
  ...
  async ['/textbook_detail']({ req, res, request, cheerio }) {
    ...
    res.send({
      ...
      data: {
        title,
        content: StringUtil.formatUnicode(content).replace('译文属可可英语原创，未经允许，不得转载。', '').replace('译文属可可原创，仅供学习交流使用，未经许可请勿转载。', '').replace(/来源:可可英语/g, '').replace(/可可/g, 'Daily每日').replace(/&apos/g, '’'),
        audio
      },
    })
  },
  ...
  async ['/renaibook_list']({ req, res, request, cheerio }) {
    ...
    const html = await request.send(`http://www.kekenet.com/chuzhong/${v}/${id == max ? '' : `List_${id}.shtml`}`)
    const $ = cheerio.load(html)
    const regx = /http:\/\/www.kekenet.com\/chuzhong\/(.*?)\.shtml/
    const result = $('#menu-list li').toArray().map((item: any) => {
      const id = $(item).find('h2').find('a').eq(0).attr('href').match(regx)[1]
      const title = $(item).find('h2').find('a').eq(0).text().trim().split(':')
      const date = $(item).find('p').text().trim().match(/(.{4})-(.{2})-(.{2})/)[0]
      // const index = $(item).find('h2').find('a').eq(1).text().trim().match(/第(.*?)期/)[1]
      return {
        // index,
  ...
  async ['/renaibook_detail']({ req, res, request, cheerio }) {
    const { id } = req.query
    const html = await request.send(`http://www.kekenet.com/chuzhong/${id}.shtml`)
    const $ = cheerio.load(html)
    const title = $('#nrtitle').text().trim()
    const content = $('#article .info-qh').html()
    const htmlmp3 = await request.send(`http://www.kekenet.com/mp3/${id}.shtml`)
